...
const defaultContacts: ContactField[] = [
  { key: "email", label: "Email", icon: Mail, placeholder: "you@seabridge.org", value: adminProfile.email, visibility: "public" },
  { key: "discord", label: "Discord", icon: MessageCircle, placeholder: "username#0000", value: adminProfile.discord, visibility: "public" },
  { key: "line", label: "LINE ID", icon: MessageCircle, placeholder: "your LINE ID", value: adminProfile.line, visibility: "private" },
...
function VisibilityToggle({ visibility, onChange }: { visibility: Visibility; onChange: (v: Visibility) => void }) {
  return (
    ...
    >
      {visibility === "public" ? <><Eye className="h-3.5 w-3.5" />Visible to fellows</> : <><EyeOff className="h-3.5 w-3.5" />Internal</>}
    </button>
  );
}
...
export default function AdminProfile() {
  ...
  const [nickname, setNickname] = useState(adminProfile.nickname);
          ...
          <Card>
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">Your details across the admin console</p>
              </div>
              {editingInfo ? (
            ...
            <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-900">{adminProfile.organization}</p>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Access level</label>
                <p className="text-sm font-medium text-slate-900">{adminProfile.accessLevel}</p>
              </div>
            </div>
          </Card>

          <Card>
            <CardHeader title="Contact Details" subtitle="Choose which contacts fellows can see" />
            ...
            <div className="flex items-center gap-2 border-t border-slate-100 bg-slate-50/50 px-5 py-3">
              ...
              <p className="text-xs text-slate-500">
                <span className="font-medium text-slate-700">Internal</span> contacts stay within the program office.{" "}
                <span className="font-medium text-slate-700">Visible</span> contacts are shown to fellows.
              </p>
            </div>
          </Card>
        </div>
